...
import pandas as pd
import spectrum.Data as Data
import sys

def handle_close(evt):
        sys.exit()
class VData(object):
    def __init__(self,data,norm,lims):
        self.data=data
        self.norm=norm
        self.minimo=lims[0]
        self.maximo=lims[1]
    ...
    def make_frame(self,t):
        self.ax.clear()
        self.ax.imshow(np.reshape(self.data[int(t),:],(16,16)).T,cmap=plt.cm.RdYlBu,interpolation='gaussian',norm=self.norm)
        self.ax.text(0.5,0.5,'max z-score=%.2f'%self.maximo)
        self.ax.text(0.5,0.8,'min z-score=%.2f'%self.minimo)
        plt.axis('off')
        plt.draw()
