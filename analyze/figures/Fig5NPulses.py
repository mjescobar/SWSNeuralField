...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr  9 11:51:52 2021
...
npulsesCL=[936/900,815/900,755/900]
npulsesP=900/900
npulsesR=799/900
#Load Baseline Data
...
#Load data
file_phase=np.load('../collectedData/CollectResults_phaseLong.npz')
...
#Plots
axC=sp.bivariable_std(collect_Iso_phase/npulsesCL,collect_so_phase/npulsesCL,axC,stdx_zero=stdSOb,meany_zero=np.mean(n_SO_white),stdy_zero=np.std(n_SO_white),marker='o',cmap=plt.cm.Dark2 ,flagstdx=True,flagstdy=True,relative_diference=False,labels=labels_phase,meanzero_type='line_inf',colormap='direct')
axC=sp.bivariable_std(collectr_Iso_frequency[:,2:3]/npulsesR,collectr_so_frequency[:,2:3]/npulsesR,axC,stdx_zero=stdSOb,meany_zero=np.mean(n_SO_white),stdy_zero=np.std(n_SO_white),marker='|',cmap=colors[1],flagstdx=True,flagstdy=True,relative_diference=False,labels=['STIM-R'],meanzero_type='None')
axC=sp.bivariable_std(collect_Iso_frequency[:,2:3]/npulsesP,collect_so_frequency[:,2:3]/npulsesP,axC,stdx_zero=stdSOb,meany_zero=np.mean(n_SO_white),stdy_zero=np.std(n_SO_white),marker='_',cmap=colors[0] ,flagstdx=True,flagstdy=True,relative_diference=False,labels=['STIM-P'],meanzero_type='None')
axC.set_ylabel('$N_{SO}$/(min.) stim',fontsize=8)
axC.set_xlabel('$I^{(SO)}$ (a. u.) stim',fontsize=8)
axC.set_xlim([0.1,0.3])
axC.set_ylim([14,22])
axC.set_yticks([14,16,18,20,22])
axC.text(-0.4,1.01,'A',fontsize=12,fontweight=1000,verticalalignment='bottom',transform=axC.transAxes)


axD=sp.bivariable_std(collect_Ispindles_phase/npulsesCL,collect_spindles_phase/npulsesCL,axD,stdx_zero=stdSpindlesb,meany_zero=np.mean(n_spindles_white),stdy_zero=np.std(n_spindles_white),marker='o',cmap=plt.cm.Dark2 ,flagstdx=True,flagstdy=True,relative_diference=False,labels=labels_phase,meanzero_type='line_inf',colormap='direct')
axD=sp.bivariable_std(collectr_Ispindles_frequency[:,2:3]/npulsesR,collectr_spindles_frequency[:,2:3]/npulsesR,axD,stdx_zero=stdSpindlesb,meany_zero=np.mean(n_spindles_white),stdy_zero=np.std(n_spindles_white),marker='|',cmap=colors[1] ,flagstdx=True,flagstdy=True,relative_diference=False,labels=['STIM-R'],meanzero_type='None')
axD=sp.bivariable_std(collect_Ispindles_frequency[:,2:3]/npulsesP,collect_spindles_frequency[:,2:3]/npulsesP,axD,stdx_zero=stdSpindlesb,meany_zero=np.mean(n_spindles_white),stdy_zero=np.std(n_spindles_white),marker='_',cmap=colors[0] ,flagstdx=True,flagstdy=True,relative_diference=False,labels=['STIM-P'],meanzero_type='None')
axD.set_ylabel('$N_{SP}$/(min.) stim',fontsize=8)
axD.set_xlabel('$I^{(SP)}$ (a. u.) stim')
axD.set_xlim([0.05,0.1])
axD.set_ylim([7,12])
axD.set_yticks([7,8,9,10,11,12])
axD.text(-0.4,1.01,'B',fontsize=12,fontweight=1000,verticalalignment='bottom',transform=axD.transAxes)


axF=sp.bivariable_std(collect_pso_phase/npulsesCL,collect_pcp_phase/npulsesCL,axF,meanx_zero=np.mean(P_SO_white),stdx_zero=np.std(P_SO_white),meany_zero=np.mean(P_PCP_white),stdy_zero=np.std(P_PCP_white),marker='o',cmap=plt.cm.Dark2,flagstdx=True,flagstdy=True,relative_diference=False,labels=labels_stimphase,meanzero_type='line_inf',colormap='direct')
axF=sp.bivariable_std(collectr_pso_frequency[:,2:3]/npulsesR,collectr_pcp_frequency[:,2:3]/npulsesR,axF,meanx_zero=np.mean(P_SO_white),stdx_zero=np.std(P_SO_white),meany_zero=np.mean(P_PCP_white),stdy_zero=np.std(P_PCP_white),marker='|',cmap=colors[1],flagstdx=True,flagstdy=True,relative_diference=False,labels=['STIM-R'],meanzero_type='None')
axF=sp.bivariable_std(collect_pso_frequency[:,2:3]/npulsesP,collect_pcp_frequency[:,2:3]/npulsesP,axF,meanx_zero=np.mean(P_SO_white),stdx_zero=np.std(P_SO_white),meany_zero=np.mean(P_PCP_white),stdy_zero=np.std(P_PCP_white),marker='_',cmap=colors[0] ,flagstdx=True,flagstdy=True,relative_diference=False,labels=['STIM-P'],meanzero_type='None')

axF.set_xlabel('$P(SO)$ stim',fontsize=8)
axF.set_ylabel('$P(C|SP)$ stim',fontsize=8)
axF.plot([0,1],[0,1],'gray')
axF.set_xlim([0.2,0.45])
axF.set_ylim([0.3,0.55])
axF.set_xticks([0.2,0.32,0.45])
axF.set_yticks([0.3,0.42,0.55])
...
#Save figure
fig.savefig('./output/Fig5Npulses.png',dpi=300,bbox_inches='tight',bbox_extra_artist=[legF])