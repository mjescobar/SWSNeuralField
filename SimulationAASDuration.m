...
Cphase=0;
stimPulseDuration=[0.05,0.075,0.1,0.15,0.3];
seeds=1:5;
...
parfor iSim=1:numSims
	...
	config=config.setStimParams(stimFrequency,stimAmplitude,stimPulseDuration(idxVar1),startTime,endTime,noiseSD,noiseMean,noiseColor,stimShape,targetPhase,stimAmplitudeSD,stimFrequencySD(indxVar3),sigmaE,sigmaI,stimX,stimY,shapeNoiseColor);
	...
	simulationPoint=['SWSAASPhin0-seed-',num2str(seeds(idxVar2)),'-Cphase',num2str(Cphase)];
	filename=util.buildFilename(simulationPoint,stimShape,stimFrequency, stimAmplitude, stimPulseDuration(idxVar1), stimAmplitudeSD, stimFrequencySD(indxVar3), plasticity_onoff);
