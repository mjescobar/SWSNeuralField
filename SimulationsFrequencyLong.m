...
%filename=util.buildFilename(simulationPoint,stimShape,stimFrequency, stimAmplitude, stimPulseDuration, stimAmplitudeSD, stimFrequencySD, plasticity_onoff,plasticityFrecuency, rateHoldLearn)
...
parfor iSim=1:numSims
	...
	config=config.setTimeParams(h,finalTime,previousTime,samplingFrequency);
	config=config.setModelParams(Nx,Ny,Lx,Ly,alpha,beta,Qmax,sigma_rho,theta,gamma,range,t0,initialStrengths,plasticity_onoff);
	endTime=ceil(180/stimFrequency(idxVar1)+5); %30 pulses.
	config=config.setStimParams(stimFrequency(idxVar1),stimAmplitude,stimPulseDuration(idxVar4),startTime,endTime,noiseSD,noiseMean,noiseColor,stimShape,targetPhase,stimAmplitudeSD,stimFrequencySD(idxVar2),sigmaE,sigmaI,stimX,stimY,shapeNoiseColor);
	config=config.setPlasticityParams(ratesA,ratesB,tp);
	stimulator=Stimulator(stimMode,config);
	model=Model(config);
	modelAAS=ModelAAS(config);
	...
	filename=util.buildFilename(simulationPoint,stimShape,stimFrequency(idxVar1), stimAmplitude, stimPulseDuration(idxVar4), stimAmplitudeSD, stimFrequencySD(idxVar2), plasticity_onoff);
	monitor=Monitor(config,['timeseries/frequency/',filename],{"Phi:E:all","Phi:N:all","Phi:R:135","Phi:S:135"});
	simulator=Simulator(config,model,modelAAS,integrator,stimulator,monitor,plasticity);
