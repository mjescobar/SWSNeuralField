...
%Variable parameters
stimShape=[0];
%stimAmplitude=[0,10,17.32,17.32,17.32,6.290,6.295,5.461,14.142,16.330,14.142,4.953,3.333]*10;
stimPulseDuration=0.05:0.01:0.3;
stimFrequencySD=[0,100];
seeds=1:20;
lenVar1=length(stimPulseDuration);
...
parfor iSim=1:numSims
	...
	stimAmplitude=sqrt(40/stimPulseDuration(idxVar1));
	config=core.Config();
	config=config.setTimeParams(h,finalTime,previousTime,samplingFrequency);
	config=config.setModelParams(Nx,Ny,Lx,Ly,alpha,beta,Qmax,sigma_rho,theta,gamma,range,t0,initialStrengths,initialChangeRates,plasticity_onoff);
	config=config.setStimParams(stimFrequency,stimAmplitude,stimPulseDuration(idxVar1),startTime,endTime,noiseSD,noiseMean,noiseColor,stimShape(idxVar4),targetPhase,stimAmplitudeSD,stimFrequencySD(idxVar2),sigmaE,sigmaI,stimX,stimY,shapeNoiseColor);
	config=config.setPlasticityParams(plasticityFrecuency,rateHoldLearn);
	stimulator=core.Stimulator(stimMode,config);
	model=core.Model(config);
	integrator=core.Integrator(config,model);
	...
	filename=core.util.buildFilename(simulationPoint,stimShape(idxVar4),stimFrequency, stimAmplitude, stimPulseDuration(idxVar1), stimAmplitudeSD, stimFrequencySD(idxVar2), plasticity_onoff,plasticityFrecuency, rateHoldLearn);
	monitor=core.Monitor(config,['timeseries/duration/',filename],{"Phi:E:all","Phi:N:all","Phi:R:135","Phi:S:135"});
	simulator=core.Simulator(config,model,integrator,stimulator,monitor,plasticity);
